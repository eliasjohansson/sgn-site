const Theme = {
  // Color
  colorPrimary: "#008484",
  colorDarkPrimary: "#005550",
  colorWhite: "#fff",
  colorLightGrey: "#F1F1F2",
  colorGrey: "#D2D2D2",
  colorDarkGrey: "#242424",
  colorBlack: "#000000",
  colorAccent: "#FD6C22",

  // Font
  fontMobileH1: "bold 28px/95% 'Noto Sans', sans-serif",
  fontMobileH2: "bold 22px/110% 'Noto Sans', sans-serif",
  fontMobileH3: "bold 18px/100% 'Noto Sans', sans-serif",
  fontMobileH4: "bold 12px/110% 'Noto Sans', sans-serif",
  fontMobilePBold: "bold 16px/120% 'Noto Sans', sans-serif",
  fontMobileP: "16px/120% 'Noto Sans', sans-serif",

  fontDesktopH1: "bold 40px/100% 'Noto Sans', sans-serif",
  fontDesktopH4: "bold 15px/100% 'Noto Sans', sans-serif",
  fontDesktopP: "18px/130% 'Noto Sans', sans-serif",

  fontMini: "11px/100% 'Noto Sans', sans-serif"
};
export default Theme;
